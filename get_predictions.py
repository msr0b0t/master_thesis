...
def predict(username):
    ...
    for tweet in timeline:
        tweet = tweet._json
        # tweets_used.append(db.tweets.insert_one(tweet).inserted_id)
    ...
    positives = np.take(feature_names, reduce(
        np.intersect1d, positive_features_used_group))
    negatives = np.take(feature_names, reduce(
        np.intersect1d, negative_features_used_group))
    ...
    # analysis_id = db.predictions.insert_one({
    #     "score": score,
    #     "tweets_used": tweets_used,
	# 	**ef.explain(positive_features, negative_features)
    # }).inserted_id
    return ({"avg_user_score": score, **ef.explain(positive_features, negative_features), "analysis_id": "blah"}) #, "analysis_id": str(analysis_id)})
